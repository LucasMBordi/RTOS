...
//	options.c_iflag &= ~(IXON | IXOFF | IXANY); // shut off xon/xoff ctrl
    ...
    //teste
    make_periodic(1000000, &info); // 1 segundos de período
// PEGAR DADOS GPS E MOSTRAR DADOS
	while(1) {
        //condição para desligar o gps
        if(state_lon == 0){
            printf("Desligando GPS...\n");
            pthread_exit(NULL);
        }
		...
		if (lineMsg[0] == '$') {
			...
			if (res == NMEA_GPGGA) {
				...
				t_long_r = (t_long * PI) / 180.0;
                              ...
                              g_time = t_gpgga.horas;
                              g_time_m = t_gpgga.minutos;
				printf("Horario: %i:%i\n", (t_gpgga.horas -3), t_gpgga.minutos);
